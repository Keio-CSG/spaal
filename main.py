from spaal2 import DummyLidarSony, DummyOutdoor, DummySpooferContinuousPulse, RealSpoofer, visualize, PreciseDuration, apply_noise, DummySpooferOff, gen_sunlight2
import time
...
def main():
    ...
    # spoofer = RealSpoofer(csv_file='data.csv', column='HistData', rows=range(3131, 3133))
    # spoofer = DummySpooferContinuousPulse(30 * 1e6, PreciseDuration(nanoseconds=5), sub_segments, debug=True)
    spoofer = DummySpooferContinuousPulse(30 * 1e6, PreciseDuration(nanoseconds=5), 'data.csv', 'HistData', 3000, 3300, debug=True)
    plotter = PlotHist2()
