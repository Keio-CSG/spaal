from spaal2 import DummyLidarSony, DummyOutdoor, DummySpooferContinuousPulse, DummySpooferOff, PreciseDuration, apply_noise, gen_sunlight
...
def calculation(
    ...
    while True:
        try:
            ...
            if sunlight_level > 0:
                sunlight = gen_sunlight(len(signal), mean=sunlight_level)
                signal = np.choose(
                    signal < sunlight,
                    [signal, sunlight]
                )
